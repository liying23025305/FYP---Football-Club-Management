...
router.get('/admin/players', isAuthenticated, isAdmin, async (req, res) => {
  ...
  let editPlayer = null;
  if (req.query.edit) {
    const [rows] = await db.execute('SELECT * FROM players WHERE player_id = ?', [req.query.edit]);
    if (rows.length > 0) editPlayer = rows[0];
  }
  res.render('admin_players', { user: req.session.user, players, editPlayer });
});

// Handle player delete
router.post('/players/delete/:id', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
...
router.post('/players/add', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
...
router.get('/admin/schedule', isAuthenticated, isAdmin, async (req, res) => {
  ...
  if (req.query.edit) {
    const [rows] = await db.execute('SELECT * FROM schedules WHERE schedule_id = ?', [req.query.edit]);
    if (rows.length > 0) editSchedule = rows[0];
  }
  res.render('admin_schedule', { user: req.session.user, schedules, editSchedule });
});

// Handle schedule update
router.post('/schedule/edit/:id', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
  const { date, title, description, location, schedule_type, team, start_time, end_time } = req.body;
  await db.execute(
    'UPDATE schedules SET start_time=?, end_time=?, title=?, description=?, location=?, schedule_type=?, team=? WHERE schedule_id=?',
    [
      `${date} ${start_time}`,
      `${date} ${end_time}`,
      title,
...
router.post('/schedule/delete/:id', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
  await db.execute('DELETE FROM schedules WHERE schedule_id=?', [req.params.id]);
  res.redirect('/admin/schedule');
});

// Handle add new schedule (if you have a form for it)
router.post('/schedule/add', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
  const { date, title, description, location, schedule_type, team, start_time, end_time } = req.body;
  await db.execute(
...
// Order more gear form
router.get('/admin/gear/order', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
  const [gear] = await db.execute('SELECT * FROM gear');
  const selectedGearId = req.query.gear_id || '';
  res.render('admin_order_gear', {
    user: req.session.user,
    gear,
    selectedGearId,
    deliveryDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10) // 7 days from now
  });
});

// Handle order more gear POST (simulate sending to supplier)
router.post('/admin/gear/order', isAuthenticated, isAdmin, async (req, res) => {
  const db = getConnection();
  const { gear_id, quantity } = req.body;
  // Update the stock in the database
  await db.execute('UPDATE gear SET gear_quantity = gear_quantity + ? WHERE gear_id = ?', [quantity, gear_id]);
  // Render a success page notifying the supplier
  const [gearRows] = await db.execute('SELECT * FROM gear WHERE gear_id = ?', [gear_id]);
  const gearItem = gearRows[0];
  res.render('admin_order_gear_success', {
    user: req.session.user,
    gearItem,
    quantity,
...
router.get('/my-orders', isAuthenticated, async (req, res) => {
  ...
  const [orders] = await db.execute(`
    SELECT o.*, p.payment_type, p.payment_status
    FROM orders o
    LEFT JOIN payments p ON o.order_id = p.order_id
    WHERE o.user_id = ?
    ORDER BY o.order_date DESC
